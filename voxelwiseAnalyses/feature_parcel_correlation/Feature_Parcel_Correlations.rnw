...
# subcortical_94x111x94.nii.gz parcel labels, for np2
# subcort.tbl <- read.csv(paste0(in.path, "subcorticalKey.csv"), stringsAsFactors=FALSE);  
...
# read node labels from the Schaefer2018_400x7 parcellation, fetched from: https://github.com/PennLINC/xcpEngine/blob/master/atlas/schaefer400x7/schaefer400x7NodeNames.txt
# read a text file, each line is a node label
sch.tbl <- read.csv(paste0(in.path, "atlas/schaefer400x7NodeNames.txt"), stringsAsFactors=FALSE, header=FALSE);
...
# TODO: find where to read subcortical data, then finish this function
