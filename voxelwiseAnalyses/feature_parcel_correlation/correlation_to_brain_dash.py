...
correlation_dir = os.path.join(root_dir, "feature_parcel_correlation/seg_xcp_24p_gsr_corrs_1_550")
...
# feature_tags = ["pcm_xcp_24p_gsr_corrs_1_550"]
# feature_name = "pixel_change_mean"
feature_tags = ["seg_xcp_24p_gsr_corrs_1_550"]
...
for feature_name in feature_names:
    for feature_tag in feature_tags:
        for i, r in enumerate(run_ids):
            if "xcp" in feature_tag:
                run_corr_sch = pd.read_table(f'{correlation_dir}/xcp_24p_gsrCorrs_{feature_name}_run{i+1}_sch.txt', sep=' ',)
                run_corr_subcortical = pd.read_table(f'{correlation_dir}/xcp_24p_gsrCorrs_{feature_name}_run{i+1}_subcortical.txt', sep=' ',)
            elif "np2" in feature_tag:
            ...
            run_corr_sch['feature_tag'] = feature_tag
            run_corr_subcortical['feature_tag'] = feature_tag
            df_sch = pd.concat([df_sch, run_corr_sch])
            df_subcortical = pd.concat([df_subcortical, run_corr_subcortical])
ids = ['sub.id', 'run', 'img', 'feature_tag', 'feature_name']
...
sch_tian_parcel = pd.read_csv(os.path.join(root_dir, "atlas/schaefer_tian_parcel.csv"))
df_sch_sub_long = pd.merge(df_sch_sub_long, sch_tian_parcel, on='parcel', how='left')
...
def update_correlation_plot(network, feature_name):
    fig = px.box(df_sch_sub_long[(df_sch_sub_long.network == network) & (df_sch_sub_long.feature_name == feature_name)], x="parcel", y="correlation")
