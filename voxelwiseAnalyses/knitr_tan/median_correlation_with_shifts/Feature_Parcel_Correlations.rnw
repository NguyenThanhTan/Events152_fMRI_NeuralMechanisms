  ...
  for (rid in run.ids) {
    ...
    for (i in 1:length(sub.ids)) {
      if (preproc == "xcp") {
        ...
        parcel.tbl <- read.np2.schaefer(sub.ids[i], rid);
         ...
         stop("Error: unknown preproc, must be either xcp or np2")
      }
      # parcel.tbl <- read.xcp.schaefer(sub.ids[i], rid);
    ...
    if (preproc == "xcp") {
      fname.out <- paste0(out_corr_dir, "xcpCorrs_", do.col, "_run", rid, "_sch.txt")
        ...
        fname.out <- paste0(out_corr_dir, "np2Corrs_", do.col, "_run", rid, "_sch.txt")
    } else {
      stop("Error: unknown preproc, must be either xcp or np2")
    }
    # fname.out <- paste0(out_corr_dir, "xcpCorrs_", do.col, "_run", rid, "_sch.txt")
...
for (do.col in movie.features) {
  ...
  for (rid in run.ids) {
    for (sid in sub.ids) {
      ...
      str <- gsub("_", " ", str)
      cat(str)
      just.plot.subject(do.col=do.col, rid=rid, sid=sid, corr_dir=out_corr_dir)
    }
    cat("\n\\newpage\n")
  }
}
