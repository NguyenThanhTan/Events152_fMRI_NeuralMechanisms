...
in_feature_dir <- paste0(in.path, "knitr_tan/median_correlation_with_shifts/out_convo_segmentation/")
...
out_corr_dir <- paste0(in.path, "knitr_tan/median_correlation_with_shifts/seg_", preproc, "_corrs_", first, "_", last, "/")
...
for (do.col in movie.features) {
  for (rid in run.ids) {
    ...
    for (do.par in c("Schaefer2018_400x7", "subcortical")) {
      ...
      if (do.par == "subcortical") {
        fname.out <- paste0(out_corr_dir, preproc, "Corrs_", do.col, "_run", rid, "_subcorticalx.txt")
      }
